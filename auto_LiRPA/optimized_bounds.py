...
def get_multinet_params(self, x, network_count, parameters, lr_lambda):
    lamb = torch.ones(network_count, dtype=torch.float, device=x[0].device, requires_grad=True)
    parameters.append({'params': lamb, 'lr': lr_lambda})
    self.optimizable_lambda = lamb
...
def extract_mininum_coef(self, lb_coef, lb_bias, ptb, unperturbed_images, indices=None):
    ...
    print(f'>>> baseline lower_bound {lower_bound.min(dim=1)[0].max()}')
...
def get_cross_network_loss(self, lb_coef, lb_bias, multi_net_target_coefs, multi_net_target_biases, ptb, unperturbed_images):
    extended_coef = torch.cat((lb_coef, multi_net_target_coefs), dim=0)
    extended_bias = torch.cat((lb_bias, multi_net_target_biases), dim=0)
    extended_coef_updated, extended_bias_updated, center, diff = self.extract_mininum_coef(extended_coef, extended_bias, ptb, unperturbed_images)
    loss = self.cross_execution_loss_helper(extended_coef_updated, extended_bias_updated,
                        extended_coef_updated.shape[0], center, diff)
    return loss
...
def get_optimized_bounds(
        ...
        multi_net_target_coefs=None, multi_net_target_biases=None):
    """
    ...
    if multiple_execution:
        if multi_net_target_coefs is not None and multi_net_target_biases is not None:
            self.get_multinet_params(multi_net_target_coefs, multi_net_target_coefs.shape[0]+1, parameters, lr_alpha)
        else: 
    ...
    for i in range(iteration):
        ...
        with torch.no_grad() if not need_grad else ExitStack():
            ...
            ret = self.compute_bounds(
                ...
                populate_trace=populate_trace, multi_net_target_coefs=multi_net_target_coefs,
                multi_net_target_biases=multi_net_target_biases)

        ret_l, ret_u = ret[0], ret[1]

        # print(f'final node name {self.final_name} input node name {self.input_node_name}')
        if multiple_execution and multi_net_target_coefs is not None and multi_net_target_biases is not None:
            lb_coef = ret[2]["final_coef"]['Final_Backprop_ANS']['lA']
            lb_bias = ret[2]["final_coef"]['Final_Backprop_ANS']['lbias']
            approx_cross_network_loss = self.get_cross_network_loss(lb_coef, lb_bias, multi_net_target_coefs, multi_net_target_biases, ptb, unperturbed_images)
            cross_execution_loss = -approx_cross_network_loss.sum() 
        elif len(ret) > 2 and multiple_execution and execution_count > 1: 
            lb_coef = ret[2]["final_coef"]['Final_Backprop_ANS']['lA']
            lb_bias = ret[2]["final_coef"]['Final_Backprop_ANS']['lbias']
        ...
        if i != iteration - 1:
            ...
            if multiple_execution and execution_count > 1:
    ...
    if len(ret) > 2 and multiple_execution and execution_count > 1: 
        lb_coef = ret[2]["final_coef"]['Final_Backprop_ANS']['lA']
        lb_bias = ret[2]["final_coef"]['Final_Backprop_ANS']['lbias']
