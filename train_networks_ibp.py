...
from training_utils import load_mnist, load_cifar10, train_ibp
...
def train_and_save_model(model, model_name, dataset_name, train_loader, test_loader, device, num_epochs=100, epsilon = 0.3):
    """
    Train a model using IBP method and save it in ONNX format
    ...
    # Train with IBP method
    model, history = train_ibp(
        model=model,
    ...
    model_path = f"project_networks/{dataset_name}_{model_name}_ibp.onnx"
